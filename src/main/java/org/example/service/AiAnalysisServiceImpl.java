...
public class AiAnalysisServiceImpl implements AiAnalysisService {
     ...
     * 4. 解析 API 返回的 JSON 格式的分析结果。
     * 5. 将分析结果（包括分析文本、建议、匹配的标签）保存到数据库中。
     * 6. 处理并保存与帖子关联的标签。
     * </p>
    ...
    public AiAnalysis analyzeMood(MoodEntry moodEntry) {
        ...
        if (geminiApiKey == null || geminiApiKey.isEmpty()) {
            logger.error("GEMINI_API_KEY 未设置。无法调用 Gemini API.\n请确保在运行环境（如IDEA环境变量、系统环境变量或Render配置）中正确设置了 GEMINI_API_KEY。");
        ...
        try {
            ...
            // 2. 构建请求体 JSON
            // 注意：这里对 promptText 中的双引号进行了转义，以确保 JSON 格式正确
            ...
            if (response.statusCode() != 200) {
                logger.error("Gemini API 调用失败。状态码: {}，响应体: {}", response.statusCode(), response.body());
                throw new RuntimeException("Gemini API 调用失败，请检查 API Key 或网络连接。");
            }
            ...
            logger.error("调用 Gemini API 时发生 IO 异常", e);
            throw new RuntimeException("调用 AI 服务失败，请稍后重试。", e);
        } catch (InterruptedException e) {
            logger.error("调用 Gemini API 时线程中断", e);
            Thread.currentThread().interrupt(); // 重新设置中断状态
            throw new RuntimeException("调用 AI 服务失败，线程中断。", e);
        } catch (JsonSyntaxException e) {
            logger.error("解析 Gemini API 返回的 JSON 时发生错误", e);
            throw new RuntimeException("解析 AI 分析结果失败，返回的 JSON 格式不正确。", e);
        } catch (Exception e) {
            logger.error("分析情绪时发生未知错误", e);
    ...
    private AiAnalysis parseAnalysis(String json, MoodEntry moodEntry) throws JsonSyntaxException {
        ...
        try {
            ...
            logger.error("解析 Gemini API 响应的嵌套 JSON 结构时发生错误: {}", e.getMessage());
            // 如果解析失败，尝试直接解析顶层 JSON，以防 Gemini 直接返回了我们期望的 JSON
            try {
                ...
                logger.error("尝试直接解析顶层 JSON 也失败: {}", ex.getMessage());
